...
from .db import Account, ApiToken, User, init_db, session_scope
...
def current_prices(max_age_s: float = 15.0) -> dict[str, float]:
    global _price_cache
    now = time.time()
...
app = FastAPI(title="Volvox Trader API", version="0.2.0", docs_url="/api/v1/docs", openapi_url="/api/v1/openapi.json")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def register(body: Credentials) -> dict[str, Any]:
    ...
    with session_scope() as s:
        ...
        token = new_token()
        s.add(ApiToken(user_id=user.id, token_hash=token_hash(token)))
